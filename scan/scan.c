...
xTaskHandle Handle_Network_bacnet;
xTaskHandle Handle_Network_modbus;
...
typedef struct
{
	...
	U8_T flag;
	U8_T retry;
}STR_NPM_NODE_OPERATE;  // network modbus points
STR_NPM_NODE_OPERATE far NPM_node_write[STACK_LEN];
...
STR_NPB_NODE_OPERATE far NPB_node_write[MAX_NPB_NODE_WRITE];
#endif
...
U8_T subcom_seq[3];
U8_T subcom_num;
U8_T flag_get_sub_port = 0;
U8_T temp_sub_port = 0;
...
void Get_Tst_DB_From_Flash(void)
{
	...
	{ // if com config can be changed automatically
		if(uart0_sub_no > 0)
		{
...
#if (ARM_MINI || ARM_CM5 || ARM_TSTAT_WIFI )
		// check whether the id is recored already.		
...
U8_T get_port_by_id(U8_T id)
{
	...
	if(flag_get_sub_port == 1)
	{
		return temp_sub_port + 1;
	}
...
// check whether current device is Power Merter, if that, deal with it speicailly, muilt-read
	...
	// only for customer test
	//delay = 50;
	
	length = wait_subnet_response(delay + 20 + read_sub_offset,port);
	...
	//					}
			...
			{					
				...
				remote_points_list[index].lose_count = 0;
					...
					for(i = 0;i < buf[5];i++)
					{
						...
						{  // modbud fuction 01 02							
							if(remote_points_list[index].tb.RP_modbus.reg > 255)
							{
								...
								LOW_BYTE(remote_points_list[index].tb.RP_modbus.reg),(subnet_response_buf[3] != 0) ? 1 : 0,
								(remote_points_list[index].tb.RP_modbus.func & 0x80) ? 1 : 0,
								float_type);
							}
							else
								add_remote_point(remote_points_list[index].tb.RP_modbus.id,remote_points_list[index].tb.RP_modbus.func & 0x1f,0,remote_points_list[index].tb.RP_modbus.reg,\
							(subnet_response_buf[3] != 0) ? 1 : 0,(remote_points_list[index].tb.RP_modbus.func & 0x80) ? 1 : 0,
							float_type);
						}
						else // 03 04
						{
					...
					for(i = 0;i < buf[5] / 2;i++)
					{
						...
						else  // T3 read 2 bytes
						{
							...
							)
							{
								...
								(remote_points_list[index].tb.RP_modbus.func & 0x80) ? 1 : 0,
								float_type);
								// check whether exist read T3-IO point in program, 
								//for special custoemr
								add_remote_point(remote_points_list[index].tb.RP_modbus.id,MB_REG + 1,
								0,remote_points_list[index].tb.RP_modbus.reg,(U32_T)(subnet_response_buf[35] << 24) + (U32_T)(subnet_response_buf[36] << 16) \
									+ (U16_T)(subnet_response_buf[33] << 8) + subnet_response_buf[34],\
								1,
								float_type);
...
//								float_type);
	...
	{	
		...
		tst_retry[buf[0]]++;
#if  T3_MAP	 	
		if(type != 0)  // read parameters
		{
			if(read_len == 0)
			{ // read one byte				
				remote_modbus_index = remote_modbus_index + buf[5] - 1;
				
				// if rmp is offline, update the value
				remote_points_list[index].lose_count++;

				if((remote_points_list[index].lose_count >= 10) && (Last_Contact_Remote_points[index] > 30))
				{
					remote_points_list[index].lose_count = 0;
					remote_points_list[index].decomisioned = 0;	
					remote_points_list[index].point_value = 0;
					if(remote_points_list[index].tb.RP_modbus.reg > 255)
					{
						add_remote_point(remote_points_list[index].tb.RP_modbus.id,
						(remote_points_list[index].tb.RP_modbus.func & 0x1f)| (HIGH_BYTE(remote_points_list[index].tb.RP_modbus.reg) << 5),
						HIGH_BYTE(remote_points_list[index].tb.RP_modbus.reg) >> 3,
						LOW_BYTE(remote_points_list[index].tb.RP_modbus.reg),
						0,						
						(remote_points_list[index].tb.RP_modbus.func & 0x80) ? 1 : 0,
						float_type);
					}
					else
					{
						...
						remote_points_list[index].tb.RP_modbus.func & 0x1f,
						0,
						remote_points_list[index].tb.RP_modbus.reg,
						0,
						(remote_points_list[index].tb.RP_modbus.func & 0x80) ? 1 : 0,
						float_type);
					}
...
void check_write_to_nodes(U8_T port)
{
	...
	port = get_port_by_id(buf[0]); // get port by id			
	baut = get_baut_by_id(port,buf[0]);
	if(port == 0) 
	{  // wrong id
	    return;
...
void Send_Shutdown(void)
{
	...
	buf[2] = 0x00;
	buf[3] = 0x10;
	buf[4] = shutdown_Time;  // high byte is time of shutdown, defautl is 5 mintues
	buf[5] = 0x55;  // write reg16 to 0x55, it is shutdown command
	...
	if(Modbus.com_config[2] == MODBUS_MASTER)
	{
		uart_init_send_com(2);
		uart_send_string(buf,8,2);
		set_subnet_parameters(RECEIVE,10,2);
	}
	else if(Modbus.com_config[0] == MODBUS_MASTER)
	{
		uart_init_send_com(0);
		uart_send_string(buf,8,0);
		set_subnet_parameters(RECEIVE,10,2);
	}
}
#endif
...
uint8_t read_PM_value(uint16_t reg)
{
	U8_T port;
	U8_T baut;
	U8_T buf[8];
...
uint8_t read_PM_value_float(uint16_t reg)
{
	U8_T port;
	U8_T baut;
	U8_T buf[8];
	...
	{		
		...
		{
//			auto_check_master_retry[port] = 0;
			if(subnet_response_buf[4] == PM_id)
			{ // PM exist
				ret = 1;
			}
			else
...
void scan_PM(void)
{
	...
	{
	case E_PM_SCAN_ID:
		ret = read_PM_value(80);
		status = E_PM_READ_RATIO;
		break;
	case E_PM_READ_RATIO:
		ret = read_PM_value(91);
		status = E_PM_READ_N_CURT;
		break;
	case E_PM_READ_N_CURT:
		ret = read_PM_value(93);
		status = E_PM_READ_RLY_SYS;
		break;
	case E_PM_READ_RLY_SYS:
		ret = read_PM_value(150);
		status = E_PM_READ_VOL_AVG;
		break;
	case E_PM_READ_VOL_AVG:
		ret = read_PM_value_float(1006);
		status = E_PM_READ_CUR_AVG;
		break;
	case E_PM_READ_CUR_AVG:
		ret = read_PM_value_float(1022);
		status = E_PM_READ_PWR_AVG;
		break;
	case E_PM_READ_PWR_AVG:
		ret = read_PM_value_float(1032);
		status = E_PM_READ_RATIO;
		break;
	default:
		break;
...
uint16_t READ_POINT_TIMER;
uint16_t READ_POINT_TIMER_FROM_EEP;
U8_T far index_sub;
...
{
	if((run_time > 120) && (force_scan == 0)) //update Read_point_timer 10min later
	{			
...
void ScanTask(void)
{
	...
	// need read at least twice ????
	E2prom_Read_Byte(EEP_READ_SUB_OFFSET,&temp);
	E2prom_Read_Byte(EEP_READ_SUB_OFFSET,&temp);
	E2prom_Read_Byte(EEP_READ_SUB_OFFSET,&temp);
	if(temp == 0xff)
	{
		temp = 0;  
		E2prom_Write_Byte(EEP_READ_SUB_OFFSET,temp);		
	}
	read_sub_offset = temp;
	
	E2prom_Read_Byte(EEP_READ_POINT_TIMER,&temp);
	if(temp == 0xff)
	{
		temp = 5;  // default 500ms
		E2prom_Write_Byte(EEP_READ_POINT_TIMER,temp);
	}
	READ_POINT_TIMER_FROM_EEP = temp * 100; //uint ms
	READ_POINT_TIMER = 100;
...
#if 0
U8_T Send_bip_Flag_COV;  // 0 - no send, 1- send , 2 - wait
U8_T Send_bip_Flag_BBMD;  // 0 - no send, 1- send , 2 - wait
U8_T far Send_bip_address_COV[6];
U8_T far Send_bip_address_BBMD[6];
U8_T far Send_bip_count_COV;
U8_T far Send_bip_count_BBMD;
U8_T far count_send_bip_COV;
U8_T far count_send_bip_BBMD;
#endif
...
void Check_Send_bip(void)
{
...
#if 0
	if(Send_bip_Flag_COV != 0)
	{	
		if(Send_bip_Flag_COV++ < 5)
			Send_bip_Flag_COV++;
		else
		{
			Send_bip_Flag_COV = 0;
	...
	if(Send_bip_Flag_BBMD != 0)
	{	
		if(Send_bip_Flag_BBMD++ < 5)
			Send_bip_Flag_BBMD++;
		else
		{
...
void Update_Subscribe_COV(uint8_t type, uint32_t instance)
{
	//Test[32]++;
}

// deal with COV data when receive ucov_notify
...
//    cov_data_value_list_link(&cov_data, &value_list[0], 1);
//    /* first value */
...
//    value_list[0].priority = 0;
    ...
    /* second value */
//		value_list[1].propertyIdentifier = PROP_PRESENT_VALUE;
//    value_list[1].propertyArrayIndex = BACNET_ARRAY_ALL;
//    bacapp_parse_application_data(BACNET_APPLICATION_TAG_REAL, "111.0",
//        &value_list[1].value);
...
//uint32_t NPB_node_write_index = 0; // Index to track the current record being processed
...
int check_NP_Bacnet_to_nodes(void)
{		
...
//	Set_bip_client_reading_flag();
	...
	while(1)
	{
		...
		if(scan_network_bacnet_count % send_whois_interval == 0)  // 1min+
		{
			...
			{// if no network points, do not scan scan command
				...
				{							
					if((run_time < 60) || (run_time - time_last_send_who_is >= 120))
					{
						// ?????????????	
	...
	#if ARM_UART_DEBUG
		printf("send bacnet who-is \r\n");
	#endif
	#if ARM_CM5				
						if(run_time > 10)  // do not 
	#endif			
						time_last_send_who_is = run_time;					
		...
		//else  // check whether reading remote point 
		{
			...
			{// get panel number from proprietary
				...
				if(remote_panel_db[i].protocal == BAC_IP && remote_panel_db[i].product_model != 2)
				{	
					...
					uart1_init(115200);
					DEBUG_EN = 1;
					printf("deviceid: %d, %lu,get panel number from proprietary  \r\n",i,remote_panel_db[i].device_id);
#endif
					temcovar_panel = 0;
					...
					uart1_init(115200);
					DEBUG_EN = 1;
					printf("index: %d, receive_netp_temcovar \r\n",i);
#endif						
...
// if update panel and sub_id, should update network points table also
			...
			{	
				...
				// check whether the device is online or offline	
						...
						wait_count = 0;
						while((flag_receive_netp == 0) && (wait_count++ < 20))
						{
							IWDG_ReloadCounter(); 
...
int8_t Get_uip_conn_pos(uip_ipaddr_t *lripaddr, u16_t lrport)
{
    ...
    conn = &uip_conns[c];
		...
		/* tcp server: must also match remote port ?? one IP can open several
		 * sockets to local Modbus port 502; ripaddr+lport alone is ambiguous */
		if((memcmp(conn->ripaddr,lripaddr,4) == 0)  
			&& (conn->lport == lrport)
			&& (conn->rport == uip_conn->rport)
...
// if first connect this ip, connect it.
	...
	{	
		...
		{// 01 03 02 04
			...
			// one custoemr need read_input, and another cusotmer need read_input_float...????
...
int write_NP_Modbus_to_nodes(U8_T ip,U8_T func,U8_T sub_id, U16_T reg, U16_T * value, U8_T len)
{
	...
	{	
		...
		{			
			...
			for(j = 0;j < len ;j++)
			{
				temp_value[j * 2] = value[j];
				temp_value[j * 2 + 1] = value[j]>> 8;
			}
			memcpy(&NPM_node_write[i].value[0],&temp_value,len * 2);
		}
		NPM_node_write[i].len = len;
...
//	uint8_t ip,sub_id,func,reg;
	...
	for(i = 0;i < STACK_LEN;i++)
	{
		if(NPM_node_write[i].flag == WAIT_FOR_WRITE) //	get current index, 1 -- WAIT_FOR_WRITE, 0 -- WRITE_OK
		{
			if(NPM_node_write[i].retry < MAX_WRITE_RETRY)
			{
			...
			{  	// retry 10 time, give up
				NPM_node_write[i].flag = WRITE_OK; 
				NPM_node_write[i].retry = 0;
				return 0;
			}
...
//		return 0;	
//	}
	uip_ipaddr(ipaddr, Modbus.ip_addr[0], Modbus.ip_addr[1], Modbus.ip_addr[2], NPM_node_write[i].ip);		//??IP?192.168.1.103
	do
	{
		conn_pos = Get_uip_conn_pos(&ipaddr, htons(Modbus.tcp_port));
		if(conn_pos == -1)
	...
	{	
		...
		Modbus_Client_Command[conn_pos][0] = NPM_node_write[i].ip;//transaction_id >> 8;
		Modbus_Client_Command[conn_pos][1] = tcp_client_transaction_id;
		Modbus_Client_Command[conn_pos][2] = 0x00;
		Modbus_Client_Command[conn_pos][3] = 0x00;
		Modbus_Client_Command[conn_pos][4] = 0x00;		
		...
		Modbus_Client_Command[conn_pos][6] = NPM_node_write[i].id;
		Modbus_Client_Command[conn_pos][7] = NPM_node_write[i].func;

		
		Modbus_Client_Command[conn_pos][8] = NPM_node_write[i].reg >> 8;
		Modbus_Client_Command[conn_pos][9] = NPM_node_write[i].reg;
		if(NPM_node_write[i].len == 1)
		{
			if(NPM_node_write[i].func == 0x06)
			{
				Modbus_Client_Command[conn_pos][10] = NPM_node_write[i].value[0] >> 8;
				Modbus_Client_Command[conn_pos][11] = NPM_node_write[i].value[0];
			}
		...
		if(NPM_node_write[i].len == 2)
		{
			...
			Modbus_Client_Command[conn_pos][13] = NPM_node_write[i].value[0];
			Modbus_Client_Command[conn_pos][14] = NPM_node_write[i].value[0] >> 8;
			Modbus_Client_Command[conn_pos][15] = NPM_node_write[i].value[1];
			Modbus_Client_Command[conn_pos][16] = NPM_node_write[i].value[1] >> 8;
...
//	sem_Modbus_Client_Send = vSemaphoreCreateBinary(0);
	...
	while(1)
	{
		...
		{			
			...
			{
#if 1	
				if(network_points_list[network_point_index].lose_count > 10)
				{
					network_points_list[network_point_index].lose_count = 0;
					network_points_list[network_point_index].decomisioned = 0;
				}
				...
				network_points_list[network_point_index].invoked_id = 
					Get_Network_Point_Modbus(network_point_index);
				if(network_points_list[network_point_index].invoked_id > 0)
				{
				// check whether the device is online or offline	
						// wait whether recieve_netp
						wait_count = 0;
...
0A FF 54 45 4D 43 4F 07 00 15 00 00 0A 00 3A 4E C1 57 20 00 00 00 00 00 BF 24
		*/
		if(buf[9] < 100)  // read   command < 100
		{
			...
			size0 = 16 + size * (buf[11] - buf[10] + 1); // ??????????xx
			delay_time = size0 + 50;
		}
	...
	{
		if(Modbus.com_config[scan_port] == BACNET_SLAVE || Modbus.com_config[scan_port] == BACNET_MASTER)
			;
		else
		{
			port = scan_port;
	...
	if(port == 0)	
		...
		set_baut_by_port(port,uart2_baudrate);
#endif	
	/* Drop stale RX before new TCP->RS485 request (parallel Poll windows). */
	uart_serial_restart(port);
	uart_init_send_com(port);	
...
//				}
...
#endif
	{ 
		/* Extra wait margin: 3 parallel Modbus Poll sessions stretch bus turnaround */
		if(length = wait_subnet_response(delay_time * 3 + read_sub_offset + 40,port))
		{ 
	...
	//			auto_check_master_retry[port] = 0;
				...
				{
					/* MBAP Length = Unit+PDU = RTU len without CRC (not size0 which includes CRC). */
					tmp_sendbuf[4] = (U8_T)((length - 2) >> 8);
					tmp_sendbuf[5] = (U8_T)(length - 2);
					memcpy(&tmp_sendbuf[6],subnet_response_buf,length - 2);	
#if (ASIX_MINI || ASIX_CM5)
			TCPIP_TcpSend(TcpSocket_ME, tmp_sendbuf, 6 + (length - 2), TCPIP_SEND_NOT_FINAL); 
#endif	

#if (ARM_MINI || ARM_CM5)
					
					memcpy(tcp_server_sendbuf,tmp_sendbuf,6 + (length - 2));
					tcp_server_sendlen = 6 + (length - 2);
#endif

#if ARM_TSTAT_WIFI || ARM_MINI
				// tbd: ????????????
			memcpy(modbus_wifi_buf,tmp_sendbuf,6 + (length - 2));
			modbus_wifi_len = 6 + (length - 2);
#endif			
				}					
				// tbd:
	...
	if((length > 0) && (length < MAX_BUF_LEN))
		memset(subnet_response_buf,0,length);
	/* Bus settle before next TCP forward (IDs 1/2/3 polled in parallel). */
	delay_ms(10);
// resuem bautrate after change it to temp baudrate
	UART_Init(port);
	set_subnet_parameters(SEND, 0,port);
...
void Change_com_config(U8_T port)
{
	...
	if(port == 0)
	{		
		E2prom_Read_Byte(EEP_COM0_CONFIG,&temp_config);	
		if(temp_config == BACNET_SLAVE || temp_config == BACNET_MASTER)
		{
			return;
		}
	}
	...
	if(port == 2)
	{		
		E2prom_Read_Byte(EEP_COM2_CONFIG,&temp_config);	
		if(temp_config == BACNET_SLAVE || temp_config == BACNET_MASTER)
			return;
	}
// if discover device in this port by hands, change config
	...
	{ // if com config can be changed automatically
		if(Modbus.com_config[port] != MODBUS_MASTER)
		{
			Modbus.com_config[port] = MODBUS_MASTER;		
			E2prom_Write_Byte(EEP_COM0_CONFIG + port, Modbus.com_config[port]);
			com_config_back[port] = Modbus.com_config[port];
			Count_com_config();
		}
	}
...
void Record_conflict_ID(U8_T id, U32_T oldsn,U32_T newsn,U8_T port,U8_T product)
{
	...
	for( i = 0; i < MAX_ID_CONFILCT;i++)
	{
		if(id == id_conflict[index_id_conflict].id)
		{
			return;
		}
	}
...
u8 count_transfer_bip_to_mstp = 0;
#if ARM_MINI	
...
//		}
//	}
	if(count_transfer_bip_to_mstp > 100)
		count_transfer_bip_to_mstp = 0;
	if(Send_mstp_Flag)
	{
		count_transfer_bip_to_mstp++;
