...
// for MSV
uint8 FAN_MODE[8][9];
U8_T far flag_count_in[HI_COMMON_CHANNEL + 4];
...
// T3-OEM-12I
#define OEM_SEL1 	PGout(14)
#define OEM_SEL2 	PGout(15)
#define OEM_SEL3 	PCout(4)
#define OEM_SEL4 	PGout(6)
#define OEM_MOD_EN PGout(13)
...
//		else
	...
	{
		high_spd_counter_tempbuf[HI_COMMON_CHANNEL] = 0;
		high_spd_counter_tempbuf[HI_COMMON_CHANNEL + 1] = 0;
		high_spd_counter_tempbuf[HI_COMMON_CHANNEL + 2] = 0;
		high_spd_counter_tempbuf[HI_COMMON_CHANNEL + 3] = 0;
	}
	else
	{
...
/*u32 check_pulse_high(u8 line)
 {
	 u32 count = 0;
	 count = 0;
	 if(line == 0)
	 {
		 while((READ_PULSE1) && (count < FILTER))  // filter pulse less than 1ms 
		 {
			 delay_us(1);
			 count++;
		 }
	 ...
	 {
		 while((READ_PULSE2) && (count < FILTER))  // filter pulse less than 1ms 
		 {
			 delay_us(1);
			 count++;
		 }
	 ...
	 {
		 while((READ_PULSE3) && (count < FILTER))  // filter pulse less than 1ms 
		 {
			 delay_us(1);
			 count++;
		 }
	 ...
	 {
		 while((READ_PULSE4) && (count < FILTER))  // filter pulse less than 1ms 
		 {
			 delay_us(1);
			 count++;
		 }
...
void EXTI15_10_IRQHandler(void)
{
	...
	{   
		...
		if(Modbus.mini_type == MINI_T3OEM_12I)
		{
			temp = HI_COMMON_CHANNEL2 - HI_COMMON_CHANNEL;		
		}
		else
			temp = 0;
	
		if((inputs[COMMON_CHANNEL + temp].range == HI_spd_count) || (inputs[COMMON_CHANNEL + temp].range == N0_2_32counts)
			|| (inputs[COMMON_CHANNEL + temp].range == RPM))
		{ 
			//if(check_pulse_high(0) >= FILTER)
			{
				high_spd_counter_tempbuf[COMMON_CHANNEL + temp]++;
			}
		}
	...
	{   Test[22]++;    
		...
		if((inputs[COMMON_CHANNEL + 1 + temp].range == HI_spd_count) || (inputs[COMMON_CHANNEL + 1 + temp].range == N0_2_32counts)
			|| (inputs[COMMON_CHANNEL + 1 + temp].range == RPM))		
		{ 
			//if(check_pulse_high(1) >= FILTER)
				high_spd_counter_tempbuf[COMMON_CHANNEL + 1 + temp]++;
		}
#if 0		
		else
		{			
	...
	{   
		...
		if((inputs[COMMON_CHANNEL + 2 + temp].range == HI_spd_count) || (inputs[COMMON_CHANNEL + 2 + temp].range == N0_2_32counts)
			|| (inputs[COMMON_CHANNEL + 2 + temp].range == RPM))		{
				
			high_spd_counter_tempbuf[COMMON_CHANNEL + 2 + temp]++;
		}
#if 0		
		else
		{				// PWM		
	...
	{      
		...
		if((inputs[COMMON_CHANNEL + 3 + temp].range == HI_spd_count) || (inputs[COMMON_CHANNEL + 3].range == N0_2_32counts)
			|| (inputs[COMMON_CHANNEL + 3 + temp].range == RPM))
		{
			high_spd_counter_tempbuf[COMMON_CHANNEL + 3 + temp]++;
		}
#if 0		
		else
		{				// PWM		
...
// for PLC, ONLY FOR CALIBRATE AO2,AO3 
U16_T new_slop[2][10];
U16_T new_start_adc[2][11];
U16_T new_vcc_adc[3];
...
void cal_slop(void)
{
	...
	for(i = 0;i < 10;i++)
	{
		...
		if(Modbus.mini_type == MINI_T3OEM_12I)
		{
			new_slop[0][i] = new_start_adc[0][i+1] - new_start_adc[0][i];
			new_slop[1][i] = new_start_adc[1][i+1] - new_start_adc[1][i];
		}
	}
...
/* add adjust output */
U16_T conver_ADC_new(U16_T *start_adc, U16_T * slop,U16_T adc)
{
	U8_T far seg;
	U16_T far real_adc = 0;
...
void initial_tstat10_range(void)
{
	...
	if(Check_sensor_exist(E_FLAG_OCC))
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 3].range = UNOCCUPIED_OCCUPIED;	
			inputs[HI_COMMON_CHANNEL2 + 3].digital_analog = 0;
			inputs[HI_COMMON_CHANNEL2 + 3].control = 0;
		}
		else
		{
	...
	else
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 3].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 3].value = 0;	
		}
		else
		{
...
// humidity & temperature	
	if(Check_sensor_exist(E_FLAG_HUM))
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 2].range = 27;  //humidity RANGE	
			inputs[HI_COMMON_CHANNEL2 + 2].digital_analog = 1;
			inputs[HI_COMMON_CHANNEL2 + 2].value = 0;
		}
		else
		{
	...
	else
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 2].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 2].value = 0;	
		}
		else
		{
	...
	{
		if(inputs[HI_COMMON_CHANNEL2].range == 0)
		{
			inputs[HI_COMMON_CHANNEL2].range = R10K_40_120DegC;	
			inputs[HI_COMMON_CHANNEL2].digital_analog = 1;
		}
	}
	else
	{
...
//	co2
	if(Check_sensor_exist(E_FLAG_CO2))
	{
		...
		{
			if(inputs[HI_COMMON_CHANNEL2 + 4].range == 0)
			{
				inputs[HI_COMMON_CHANNEL2 + 4].range = 28;  //????????????	
				inputs[HI_COMMON_CHANNEL2 + 4].digital_analog = 1;
				inputs[HI_COMMON_CHANNEL2 + 4].value = 0;
			}
		}
		else
		{
	...
	else
	{
		...
		{			
			inputs[HI_COMMON_CHANNEL2 + 4].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 4].value = 0;	
		}
		else
		{
	...
	if(Modbus.mini_type == MINI_T3OEM_12I)
	{
		for(i = 0;i < 4;i++)
		{
			if(inputs[12 + i].range == 0)
			{
				inputs[12 + i].range = HI_spd_count;	
				inputs[12 + i].digital_analog = 1;
			}
		}
	}
	...
	if(Check_sensor_exist(E_FLAG_TVOC))
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 1].range = PPB;  // tbd: add PPB
			inputs[HI_COMMON_CHANNEL2 + 1].digital_analog = 1;
			inputs[HI_COMMON_CHANNEL2 + 1].value = 0;
		}
		else
		{
	...
	else
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 1].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 1].value = 0;	
		}
		else
		{
	...
	if(Check_sensor_exist(E_FLAG_LIGHT))
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 5].range = LUX;  
			inputs[HI_COMMON_CHANNEL2 + 5].digital_analog = 1;
			inputs[HI_COMMON_CHANNEL2 + 5].value = 0;
		}
		else
		{
	...
	else
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 5].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 5].value = 0;	
		}
		else
		{
	...
	if(Check_sensor_exist(E_FLAG_VOICE))
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 6].range = DB;  // DB
			inputs[HI_COMMON_CHANNEL2 + 6].digital_analog = 1;
			inputs[HI_COMMON_CHANNEL2 + 6].value = 0;
		}
		else
		{
	...
	else
	{
		...
		{
			inputs[HI_COMMON_CHANNEL2 + 6].range = 0;	
			inputs[HI_COMMON_CHANNEL2 + 6].value = 0;	
		}
		else
		{
...
void Input_IO_Init(void)
{
	...
	if(Modbus.mini_type == MINI_T3OEM_12I)
	{
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15 | GPIO_Pin_6 | GPIO_Pin_14 | GPIO_Pin_13; 	
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(GPIOG, &GPIO_InitStructure);
		GPIO_ResetBits(GPIOG, GPIO_Pin_15 | GPIO_Pin_6 | GPIO_Pin_14);	
...
//	static u8 count_no_mic = 0;
	...
	{
		temp2 = Filter(HI_COMMON_CHANNEL2,temp2 / 400);
		temp3 = Filter(HI_COMMON_CHANNEL2 + 5,temp3 / 100);
		temp4 = Filter(HI_COMMON_CHANNEL2 + 3,temp4 / 100);  // occ
	}
	else
	{
	...
	{
		if(channel_count == 1)
			input_raw[channel_count] = (U32_T)temp1 * 1023 / new_vcc_adc[0];
		else if(channel_count == 2)
			input_raw[channel_count] = (U32_T)temp1 * 1023 / new_vcc_adc[1];
		else if(channel_count == 7)
			input_raw[channel_count] = (U32_T)temp1 * 1023 / new_vcc_adc[2];
		else
	...
	if(Modbus.mini_type == MINI_T10P)
		...
		input_raw[HI_COMMON_CHANNEL2] = temp2;		// temperatue
	else
	...
	if(Check_sensor_exist(E_FLAG_LIGHT))
	{
		...
		else if(Modbus.mini_type == MINI_T3OEM_12I)
		{
			...
			temp3 = 3000 * temp3 / 4095;
			light_temp = temp3 * 100 / (LIGHT_COE * LIGHT_R1);
			inputs[HI_COMMON_CHANNEL2 + 5].value = light_temp * 1000;				
		}
		else
		{
	...
	if(Check_sensor_exist(E_FLAG_OCC))
	{
		...
		{
			if(inputs[HI_COMMON_CHANNEL2 + 3].digital_analog == 0)
			{
				if(abs(temp4 * 3000 / 40 / 1023 - PirSensorZero) > Pir_Sensetivity) //occupied
				{
					inputs[HI_COMMON_CHANNEL2 + 3].control = 1;  // OCC
					override_timer_time = (uint32)60 * override_timer;
				}				
			}
			else
			{
				inputs[HI_COMMON_CHANNEL2 + 3].value = temp4 * 3000 / 40 / 1023;
			}
		}
		else
		{
	...
	if(Modbus.mini_type == MINI_T3OEM_12I)
	{
		...
		{
			case 0:
				OEM_SEL1 = 0;OEM_SEL2 = 0;OEM_SEL3 = 0;OEM_SEL4 = 0;
				break;
			case 1:
				OEM_SEL1 = 1;OEM_SEL2 = 0;OEM_SEL3 = 0;OEM_SEL4 = 0;
				break;
			case 2:
				OEM_SEL1 = 0;OEM_SEL2 = 1;OEM_SEL3 = 0;OEM_SEL4 = 0;
				break;
			case 3:
				OEM_SEL1 = 1;OEM_SEL2 = 1;OEM_SEL3 = 0;OEM_SEL4 = 0;
				break;
			case 4:
				OEM_SEL1 = 0;OEM_SEL2 = 0;OEM_SEL3 = 1;OEM_SEL4 = 0;
				break;
			case 5:
				OEM_SEL1 = 1;OEM_SEL2 = 0;OEM_SEL3 = 1;OEM_SEL4 = 0;
				break;
			case 6:
				OEM_SEL1 = 0;OEM_SEL2 = 1;OEM_SEL3 = 1;OEM_SEL4 = 0;
				break;
			case 7:
				OEM_SEL1 = 1;OEM_SEL2 = 1;OEM_SEL3 = 1;OEM_SEL4 = 0;
				break;
			case 8:
				OEM_SEL1 = 0;OEM_SEL2 = 0;OEM_SEL3 = 0;OEM_SEL4 = 1;
				break;
			case 9:
				OEM_SEL1 = 1;OEM_SEL2 = 0;OEM_SEL3 = 0;OEM_SEL4 = 1;
				break;
			case 10:
				OEM_SEL1 = 0;OEM_SEL2 = 1;OEM_SEL3 = 0;OEM_SEL4 = 1;
				break;
			case 11:
				OEM_SEL1 = 1;OEM_SEL2 = 1;OEM_SEL3 = 0;OEM_SEL4 = 1;
				break;
			default:
				break;
		}
...
//REALY 3 4		
	...
	if(Modbus.mini_type == MINI_T3OEM_12I)
	{ // PB9 AO->BO in customized product
	// REALY6 PB9
...
U16_T conver_TCU_MINI_ADC(U8_T refresh_index, U16_T adc1)
{
	U16_T adc;
	if(refresh_index == 1)
	{
		conver_ADC_new(new_start_adc[0],new_slop[0],adc1);
	}
	else if(refresh_index == 2)
	{
		conver_ADC_new(new_start_adc[1],new_slop[1],adc1);
	}
	else
		adc = conver_ADC(adc1);
...
//	U32_T far adc2; /* adc1 is for the first 4051, adc2 is for the second 4051 */
	...
	// base out, TSTAT10 is 5, T3_OEM_12I is 7
	if(outputs[refresh_index + max_dos].digital_analog == 0) // digital
	{
		if(output_raw[refresh_index + max_dos] >= 512)
	...
	{
		adc1 = output_raw[refresh_index + max_dos];
...
//		{
			if(Modbus.mini_type == MINI_T3OEM_12I)
			{ // Calibrate each output channel separately
				adc1 = conver_TCU_MINI_ADC(refresh_index,adc1);
			}
			else
...
// if output is used for a digtal output, do not use feedback to adjust
	...
	if(Modbus.mini_type == MINI_T3OEM_12I)
	{
		if(test_adc_flag & 0x0001) 
		{
			uint8_t channel;
			channel = (U8_T)(test_adc_flag >> 8);
			if(refresh_index == channel)
				adc1 = test_adc;
		}
	}
	else
	{
		//only calibrate the first channel
		if(refresh_index == 0) 
		{
		...
		if(outputs[refresh_index + max_dos].range == 0)		
			adc1 = 0;
...
{
	Output_IO_Init();
	sTaskCreate( refresh_Input_Task, "InputTask1", 500, NULL, uxPriority, &xHandler_Input );
...
void Hum_Initial(void)
{
	...
	if(hum_sensor_type == 0) // check whether SHT4X
	{
		hum_sensor_type = SHT4x_Initial(); 
	}
	Test[22] = hum_sensor_type + 10;
}
...
//	error = SHT3X_GetTempAndHumi(&tem_org, &hum_org, REPEATAB_HIGH, MODE_POLLING, 50);
	// if no internal temperature sensor, use temperature of humidity
	if(hum_sensor_type == 1) // SHT3X
	{
		error = SHT3X_GetTempAndHumi(&tem_org, &hum_org, REPEATAB_HIGH, MODE_POLLING, 50);
	}
	else //if(hum_sensor_type == 2) // SHT4X
	{
		error = Refresh_SHT4x();
	}	
	...
	{		
		if(Modbus.mini_type == MINI_T10P)
		{
			if(input_raw[HI_COMMON_CHANNEL] > 1000)
			{
				...
				if(!inputs[HI_COMMON_CHANNEL].calibration_sign)
				{
					inputs[HI_COMMON_CHANNEL].value = tem_org * 100 + 100L * (inputs[HI_COMMON_CHANNEL].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL].calibration_lo);
				}
				else
				{
					inputs[HI_COMMON_CHANNEL].value = tem_org * 100 - 100L * (inputs[HI_COMMON_CHANNEL].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL].calibration_lo);
			...
			if(!inputs[HI_COMMON_CHANNEL + 2].calibration_sign)
			{
				inputs[HI_COMMON_CHANNEL + 2].value = hum_org * 100 + 100L * (inputs[HI_COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 2].calibration_lo);
			}
			else
			{
				inputs[HI_COMMON_CHANNEL + 2].value = hum_org * 100 - 100L * (inputs[HI_COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 2].calibration_lo);
		...
		else
		{
			if(input_raw[COMMON_CHANNEL] > 1000)
			{
				...
				if(!inputs[COMMON_CHANNEL].calibration_sign)
				{
					inputs[COMMON_CHANNEL].value = tem_org * 100 + 100L * (inputs[COMMON_CHANNEL].calibration_hi * 256 + inputs[COMMON_CHANNEL].calibration_lo);
				}
				else
				{
					inputs[COMMON_CHANNEL].value = tem_org * 100 - 100L * (inputs[COMMON_CHANNEL].calibration_hi * 256 + inputs[COMMON_CHANNEL].calibration_lo);
			...
			if(!inputs[COMMON_CHANNEL + 2].calibration_sign)
			{
				inputs[COMMON_CHANNEL + 2].value = hum_org * 100 + 100L * (inputs[COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[COMMON_CHANNEL + 2].calibration_lo);
			}
			else
			{
				inputs[COMMON_CHANNEL + 2].value = hum_org * 100 - 100L * (inputs[COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[COMMON_CHANNEL + 2].calibration_lo);
...
{	
	...
	Hum_Initial();
	VOC_Init();
	if((Modbus.mini_type == MINI_T10P) ||	Modbus.mini_type == MINI_T3OEM_12I)
	{
		pulse_set();
	}	
	if(Check_sensor_exist(E_FLAG_CO2))	
	...
	HumSensor_dew_pt = 0;
	HumSensor_dew_pt_F = 0;
	HumSensor_Pws = 0;
	HumSensor_Mix_Ratio = 0;
	HumSensor_Enthalpy = 0;
	for(;;)
	{		Test[10]++;	
		...
		if(hum_org != 0)
		{
			Get_Hum_Para( tem_org, hum_org,&HumSensor_dew_pt,\
						&HumSensor_Pws,&HumSensor_Mix_Ratio,&HumSensor_Enthalpy); 
			HumSensor_dew_pt_F = (float)HumSensor_dew_pt * 9.0 / 5 + 320;
...
// OUTPUT
		...
		if(temp1.word != relay_value.word)
		{
			...
			if(Modbus.mini_type == MINI_T3OEM_12I)
			{
				if(relay_value.byte[0] & 0x20) 					TSTAT_REALY6 = 1;				else					TSTAT_REALY6 = 0;
				if(relay_value.byte[0] & 0x40) 					TSTAT_REALY7 = 1;				else					TSTAT_REALY7 = 0;
			}
		}
...
void SCD40_get_value(uint16_t co2,int32_t temperaute, int32_t humidity)
{
	...
	{		
		if(input_raw[HI_COMMON_CHANNEL] > 1000)
		{
			//inputs[HI_COMMON_CHANNEL].value = temperaute;
			if(!inputs[HI_COMMON_CHANNEL].calibration_sign)
			{
				inputs[HI_COMMON_CHANNEL].value = temperaute + 100L * (inputs[HI_COMMON_CHANNEL].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL].calibration_lo);
			}
			else
			{
				inputs[HI_COMMON_CHANNEL].value = temperaute - 100L * (inputs[HI_COMMON_CHANNEL].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL].calibration_lo);
		...
		//inputs[HI_COMMON_CHANNEL + 2].value = humidity;
		//inputs[HI_COMMON_CHANNEL + 4].value = co2 * 1000;	
		if(!inputs[HI_COMMON_CHANNEL + 2].calibration_sign)
		{
			inputs[HI_COMMON_CHANNEL + 2].value = humidity + 100L * (inputs[HI_COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 2].calibration_lo);
		}
		else
		{
			inputs[HI_COMMON_CHANNEL + 2].value = humidity - 100L * (inputs[HI_COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 2].calibration_lo);
		...
		if(!inputs[HI_COMMON_CHANNEL + 4].calibration_sign)
		{
			inputs[HI_COMMON_CHANNEL + 4].value = co2 * 1000 + 100L * (inputs[HI_COMMON_CHANNEL + 4].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 4].calibration_lo);
		}
		else
		{
			inputs[HI_COMMON_CHANNEL + 4].value = co2 * 1000 - 100L * (inputs[HI_COMMON_CHANNEL + 4].calibration_hi * 256 + inputs[HI_COMMON_CHANNEL + 4].calibration_lo);
	...
	else
	{
		if(input_raw[COMMON_CHANNEL] > 1000)
		{
			//inputs[COMMON_CHANNEL].value = temperaute;
			if(!inputs[COMMON_CHANNEL].calibration_sign)
			{
				inputs[COMMON_CHANNEL].value = temperaute + 100L * (inputs[COMMON_CHANNEL].calibration_hi * 256 + inputs[COMMON_CHANNEL].calibration_lo);
			}
			else
			{
				inputs[COMMON_CHANNEL].value = temperaute - 100L * (inputs[COMMON_CHANNEL].calibration_hi * 256 + inputs[COMMON_CHANNEL].calibration_lo);
		...
		//inputs[COMMON_CHANNEL + 2].value = humidity;
		//inputs[COMMON_CHANNEL + 4].value = co2 * 1000;	
		if(!inputs[COMMON_CHANNEL + 2].calibration_sign)
		{
			inputs[COMMON_CHANNEL + 2].value = humidity + 100L * (inputs[COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[COMMON_CHANNEL + 2].calibration_lo);
		}
		else
		{
			inputs[COMMON_CHANNEL + 2].value = humidity - 100L * (inputs[COMMON_CHANNEL + 2].calibration_hi * 256 + inputs[COMMON_CHANNEL + 2].calibration_lo);
		...
		if(!inputs[COMMON_CHANNEL + 4].calibration_sign)
		{
			inputs[COMMON_CHANNEL + 4].value = co2 * 1000 + 100L * (inputs[COMMON_CHANNEL + 4].calibration_hi * 256 + inputs[COMMON_CHANNEL + 4].calibration_lo);
		}
		else
		{
			inputs[COMMON_CHANNEL + 4].value = co2 * 1000 - 100L * (inputs[COMMON_CHANNEL + 4].calibration_hi * 256 + inputs[COMMON_CHANNEL + 4].calibration_lo);
...
void CO2_IO_Init(void)
{
	// tbd:
	// current hardware dont have reset pin
}
