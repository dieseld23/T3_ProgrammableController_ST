...
{	
	char mstp_port;
#if ARM_MINI	
	mstp_port = get_current_mstp_port();
...
char* get_description(uint8_t type,uint8_t num)
{
	...
	if(type == TEMCOAV)
	{
		...
		else if(num == 6) return "range_IN1";
		else if(num == 7) return "range_IN2";
		else if(num == 8) return "range_IN3";
		else if(num == 9) return "range_IN4";
		else if(num == 10) return "range_IN5";
		else if(num == 11) return "range_IN6";
		else if(num == 12) return "range_IN7";
		else if(num == 13) return "range_IN8";
#else
...
char* get_label(uint8_t type,uint8_t num)
{
	...
	if(type == TEMCOAV)
	{
		...
		else if(num == 4) return "lcd configure";
		else if(num == 5) return "icon configure";
		else if(num == 6) return "range_IN1";
		else if(num == 7) return "range_IN2";
		else if(num == 8) return "range_IN3";
...
// add unit
	...
	if(type == AI)	
	{
		...
		else  // analog
		{
			...
			if((inputs[io_index].range == R10K_40_120DegC) || (inputs[io_index].range == KM10K_40_120DegC) || (inputs[io_index].range == PT1000_200_300DegC)) 
				return UNITS_DEGREES_CELSIUS;
			else if((inputs[io_index].range == R10K_40_250DegF) || (inputs[io_index].range == KM10K_40_250DegF) || (inputs[io_index].range == PT1000_200_570DegF)) 
				...
				return UNITS_VOLTS;
			else if((inputs[io_index].range == P0_100_0_10V) || (inputs[io_index].range == P0_100_0_5V) || (inputs[io_index].range == P0_100_4_20ma)) 
				return UNITS_PERCENT;
			else if(inputs[io_index].range == 26/*HZ*/)  
				return UNITS_HERTZ;
			else if(inputs[io_index].range == 27 /*HUMIDTY */)  
				return UNITS_PERCENT_RELATIVE_HUMIDITY;
			else if(inputs[io_index].range == 28 /*CO2 PPM */)  
...
{	
	...
	{
		Get_index_by_AIx(num,&io_index);
		if(io_index >= MAX_AVS) return 1;
		if(inputs[io_index].auto_manual == 1)
			return 1;
		else 
			return (inputs[io_index].decom & 0x0f);
	}
	else if(type == BI)	
	{
		Get_index_by_BIx(num,&io_index);
...
//					case 4:		value = Modbus.display_lcd;		break;
					...
					case 6: value = (inputs[0].digital_analog << 8) + inputs[0].range;					break;
					case 7: value = (inputs[1].digital_analog << 8) + inputs[1].range;					break;
					case 8: value = (inputs[2].digital_analog << 8) + inputs[2].range;					break;
					case 9: value = (inputs[3].digital_analog << 8) + inputs[3].range;					break;
					case 10: value = (inputs[4].digital_analog << 8) + inputs[4].range;					break;
					case 11: value = (inputs[5].digital_analog << 8) + inputs[5].range;					break;
					case 12: value = (inputs[6].digital_analog << 8) + inputs[6].range;					break;
					case 13: value = (inputs[7].digital_analog << 8) + inputs[7].range;					break;
#else
...
//					inputs[i].control = value ? 1 : 0;	
			...
			write_page_en[OUT] = 1;
				...
				if(io_index < max_aos + max_dos)
				{
					...
					Set_AO_raw(io_index,(float)outputs[io_index].value);
				}
...
//				outputs[out_index].value = swap_double(Analog_Output_Present_Value(out_index) * 1000);//swap_double(value * 1000);
		 ...
		 case SCHEDULE:
			...
			else if(i == 5)
			{
				...
				E2prom_Write_Byte(EEP_T10_ICON_CONFIG,value);
			}
			else if(i >= 6 && i<= 13)
			{				
				inputs[i - 6].digital_analog = (U16_T)value >> 8;
				inputs[i - 6].range = (U8_T)value;
				write_page_en[IN] = 1;	
				ChangeFlash = 1;
			}
			break;
...
void write_annual_date(uint8_t index,BACNET_DATE date)
{
	...
	day += date.day;
	ar_dates[index][(day) / 8] |= (0x01 << ((day) % 8));
    ...
    char *new_name)
{
	...
	{
		memcpy(&FAN_MODE[state_index],new_name,9);
	}
...
bool Analog_Input_Change_Of_Value(unsigned int object_instance)
{
	...
	if(inputs[object_index].value != backup_IN_value[object_index])
	{
...
{
	U8_T temp;				
	if(flag_suspend_mstp == 1) 
	{	
		temp = ((count_suspend_mstp / 60) & 0x7f) + 0x80;
		AT24CXX_WriteOneByte(EEP_SUSPEND_MSTP,temp);
	}
}
...
//	{
//		Recievebuf_Initialize(2);dlmstp_init(NULL);
//		//Send_I_Am_Flag = 1;
//	}
...
//	{
//		Recievebuf_Initialize(0);dlmstp_init(NULL);
//		//Send_I_Am_Flag = 1;
//	}
#endif
}
