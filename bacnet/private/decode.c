...
//uint8_t check_point_stucture(U8_T * prog);
extern UN_Time Rtc;
...
float test_match_custom( uint8_t range, S16_T raw );
...
S16_T exec_program(S16_T current_prg, U8_T *prog_code)
{
	Point p_var;
	Point_Net point_net;
	Point_Bacnet point_bacnet;
//	uint8_t new_point_Str = 0;
	S32_T val1, val2, step;
	S16_T nbytes;
...
//	timeout = 0;
	...
	while((*prog != 0xfe) && (temp++ < 2000))
	{
		...
		case DISABLEX:
			id = *(prog-1);
			type_var = 0; /* must init ?? else garbage may equal LOCAL_POINT_PRG and wild-write OUT/IN/VAR */
			...
			else
			{
				if (*prog == REMOTE_POINT_PRG)
				{
					...
					// check whether new structure
/*					new_point_Str = check_point_stucture(prog);
					if(new_point_Str == 0)
					{*/
...
/*					}
					...
					{
						point_bacnet = *((Point_Bacnet *)prog);
						prog += sizeof(Point_Bacnet);
					}*/
				}
			}
			if ( id==OPEN )
...
//				else
//					put_net_point_value_new(&point_net,&value,ana_dig,PROGR,0);
			}
			break;
//		case PHONE:
//								len = *prog++;
							 ...
							 {	
								...
								if (cond)         /* test condition*/
									...
									{      /* condition is false*/
										...
										prog += len;
										value = swap_double(value);  // for asix
									  memcpy(prog,&value,4);
	...
	/*											interpret(); */
		...
		case WAIT:
								...
								else
								{
									value = swap_double(value);
									memcpy(prog,&value,4);
									*((S16_T *)(p_buf + nbytes + 1)) = swap_word(return_pointer - p_buf);
									return 1;
								}
								prog += 4;
								break;
...
//	S8_T *p;
	...
	{	
		...
		case BITOR:
							 op1 = pop(); op2 = pop();
							 op1 = swap_double(op1) / 1000;
							 op2 = swap_double(op2) / 1000; 							 
							 push(swap_double( (op1 | op2) * 1000));
							 break;
		case BITAND:
							 op1 = pop(); op2 = pop();
							 op1 = swap_double(op1) / 1000;
...
//							 push( ( GetByteBit(sptr.pin.in_sen_off,1)) ? 1000L : 0L );
		...
		case INTERVAL:
							...
							break;
	 	case TIME_ON:  
							pn = (S32_T *)(time_buf + swap_word((*(S16_T *)prog)));
							if( *((S8_T *)pn - 1) )
							 push(swap_double(*pn));
							else
							 push(0);
...
/*		case PVAR:  // NO USED
				op1 = pop() / 1000;
				if(op1 < 4)
					push(swap_double(net_health[op1] * 1000));
				else
					push(0);
				break;*/
	 ...
	 case MIN:
				...
				break;
	 case PIDPROP:
				m = swap_double(pop()) / 1000;
				i = swap_double(pop()) / 1000;
	 ...
	 case PIDINT:  // item
				 ...
				 push(swap_double(n * 1000));
				 break;
	 case Tbl: // TBL( expression , table# )
				op1 = (int)pop() / 1000; // 
				op2 = (int)pop() / 1000; // table number
				if(op1 < 1) op1 = 1;					
				if(op1 > 5) op1 = 5;
				push(10000l * test_match_custom(table1 + op1 - 1, op2));				
				break;
	 case WR_ON:
	 case WR_OFF:
				 op1 = pop();
...
S32_T operand(S8_T **buf,U8_T *local)
{
	...
	{	
		...
		/*	else
			{
				//get_net_point_value_new( ( (Point_Bacnet *)(++prog) ), &value,0,0 );
				prog += sizeof(Point_Bacnet);
			}*/
		}	
...
void push(S32_T value)
{
	/* stack[20] ?? overflow smashes adjacent far BSS (can look like RAM_ERR / 0xFF) */
	if(index_stack >= &stack[20])
		return;
 	*index_stack++ = value;
}
...
//	Test[15]++;
//	memcpy(&Test[16],&id,4);
	if(id == 0xff55ff55) // new bacnet structure
		return 1;
	else
		return 0;
}
