...
extern uint8_t flag_start_scan_modbus;
extern uint8_t start_scan_modbus_count;
extern uint16_t Master_Scan_modbus_Count;
...
//Str_item_Json					far			group_Json_data[];
Str_grp_element_new   far		group_data_new;
...
//float far value_priority[MAX_OUTS][16];
...
long			 far   					arrays_data[MAX_ARRAYS_DATA];
...
#endif
REMOTE_POINTS		  far  		  remote_points_list[MAXREMOTEPOINTS];
...
POINTS_HEADER 	far	local_points_list[MAXLOCALPOINTS];
Byte				far 	number_of_local_points;

U16_T Last_Contact_Local_points[MAXLOCALPOINTS];
...
//	memset( group_data, '\0', MAX_ELEMENTS * sizeof( Str_grp_element) );
	...
	{
		memset(&local_points_list[i],0,sizeof(POINTS_HEADER));
	}
	number_of_local_points = 0;
#endif
...
void Sync_Panel_Info(void)
{
  uint8 temp[2];
	...
	Panel_Info.reg.sw = SW_REV / 100 + (U16_T)((SW_REV % 100) << 8);
	Setting_Info.reg.mstp_network_number = (mstp_network);
	Setting_Info.reg.pro_info.firmware_asix = SW_REV / 100 + (U16_T)((SW_REV % 100) << 8);
...
//	Setting_Info.reg.uart_WordLen[0] = Modbus.uart_WordLen[0];
//	Setting_Info.reg.uart_WordLen[1] = Modbus.uart_WordLen[1];
//	Setting_Info.reg.uart_WordLen[2] = Modbus.uart_WordLen[2];

	Setting_Info.reg.write_flash = Modbus.refresh_flash_timer;
...
#endif
	Setting_Info.reg.fix_com_config = Modbus.fix_com_config;
}
...
uint8_t flag_send_UserList_Broadcast;
uint8_t	count_send_UserList_Broadcast;
void Check_send_UserList_Broadcast(void)
{
	if(flag_send_UserList_Broadcast)
	{
		count_send_UserList_Broadcast++;
		if(count_send_UserList_Broadcast > 20)
		{
			if(Modbus.en_username == 1)
			{
				if(Modbus.network_master == 1)
				{
					Send_UserList_Broadcast(0,7);
			...
			flag_send_UserList_Broadcast = 0;
		}
	}
}
#endif
	...
	if(((protocal == BAC_IP) ||(protocal == BAC_IP_CLIENT))  && temcoproduct == 1) // current device is temco device, and it is not subdevice
	{
		...
		if(i == remote_panel_num)  // add new panel into DB
		{
			...
			if(protocal == BAC_IP && src->len == 0 && temcoproduct == 1)
				// current device is temco device, and it is not sub device
			{
				if(panel == 255)  // ???????
				{
...
#if (ARM_MINI || ASIX_MINI || ARM_CM5)
					// check whether send UserList_Broadcast
...
#if (ARM_MINI || ARM_CM5 || ARM_TSTAT_WIFI )
extern int count_update_mstp_db;
void update_mstp_db(uint8_t mstp_id)
{
	U8_T i;
	STR_REMOTE_PANEL_DB * ptr;
	ptr = &remote_panel_db[0];
	count_update_mstp_db = 1000;
	for(i = 0;i < remote_panel_num;i++,ptr++)
	{
		if((mstp_id == ptr->sub_id) && (ptr->protocal == BAC_MSTP))
		{
			ptr->time_to_live = RMP_TIME_TO_LIVE;
		}
...
S16_T get_point_info_by_instacne(Point_Net * point)
{
	...
	{  // R&W modbus points in program code.	
		
		if((Modbus.com_config[0] == MODBUS_MASTER) || (Modbus.com_config[2] == MODBUS_MASTER))
		{
			// dont change com config
		}
		else
		{
			...
			if((Modbus.com_config[0] == NOUSE) && (Modbus.com_config[2] == NOUSE))
			{
				if(Modbus.fix_com_config == 0)
				{
//					if(count_modbus_slave[0] == 0)
...
#endif
		point->panel = 0;
		// not in the database, still get instance and show it in network points table
		//return instance;
	}
	return -1;	
}
...
void Send_SUB_I_Am(uint8_t index)
{
	...
	mtu[7] = 0xff;
	mtu[8] = 0x00;
	mtu[9] = (U8_T)(mstp_network/*Modbus.network_number*/ >> 8);
	
	mtu[10] = mstp_network/*Modbus.network_number*/;  // network address  10
...
// 22 xx xx -> max apdu len
// 91 03 -> no segment
// 21 xx / 22 xx xx -> vendor id
	len = 16;

//	memcpy(&mtu[len],remote_panel_db[index].remote_iam_buf,\
//	remote_panel_db[index].remote_iam_buf_len);
//	len += remote_panel_db[index].remote_iam_buf_len;
...
// vendor id	
		...
		{				
//			//if(remote_panel_db[index].sub_id == 0x10)
//			{
				mtu[len++] = 0x21;
				mtu[len++] = 0x94;
//			}
...
//			flag_uip_test = 1;
//			sprintf(uip_test,"ready to send %d instance = %u, port = %u", \
			rec_mstp_index,remote_panel_db[rec_mstp_index].device_id,uip_udp_conn->rport);
...
uint8_t flag_mstp_err[3] = {0,0,0};
uint16_t count_mstp_err[3] = {0,0,0};

#define CHECK_MSTP_INTERVAL 3
...
#if 0
	static uint32_t back_packet_err[3] = {0 , 0 , 0};
	static uint8_t sec = 0;
	uint16_t packet_count_per_sec = 0;
	
	if(sec++ % CHECK_MSTP_INTERVAL == 0)
	{	
		if(Modbus.com_config[2] == BACNET_SLAVE || Modbus.com_config[2] == BACNET_MASTER)
		{
			if(back_packet_err[2] != packet_error[2])
			{
				if(packet_error[2] - back_packet_err[2] > 30 * CHECK_MSTP_INTERVAL)
				{
					// check baudrate
					if(count_mstp_err[2]++ > 3)
						flag_mstp_err[2] = 1;
				}
				else
					Test[27]++;
					
				back_packet_err[2] = packet_error[2];
			}
			else
			{
				flag_mstp_err[2] = 0;
		...
		{
			if(back_packet_err[0] != packet_error[0])
			{
				if(packet_error[0] - back_packet_err[0] > 30 * CHECK_MSTP_INTERVAL)
				{
					// check baudrate
					if(count_mstp_err[0]++ > 3)
						flag_mstp_err[0] = 1;		
				}
				else
					Test[27]++;
					
				back_packet_err[0] = packet_error[0];
			}
			else
			{				
				flag_mstp_err[0] = 0;
