...
uint16_t BIP_Port = 0;   /* this will force initialization in demos */
 ...
 //   (void) npdu_data;
    ...
    mtu_len += pdu_len;
		...
		if(Send_bip_Flag)
		{
			...
			bip_send_client_conn = uip_udp_new(&addr, HTONS(BIP_Port/*Send_bip_address[4] * 256 + Send_bip_address[5]*/)); // des port
			if(bip_send_client_conn != NULL) 
			{
...
// for COV
#if 0
struct uip_udp_conn * bip_send_client_conn_COV;
int bip_send_pdu_client_COV(
    BACNET_ADDRESS * dest,      /* destination address */
    BACNET_NPDU_DATA * npdu_data,       /* network information */
    uint8_t * pdu,      /* any data to be sent - may be null */
...
{       /* number of bytes of data */  
 ...
 //   (void) npdu_data;
    ...
    memcpy(&mtu1[mtu_len], pdu, pdu_len);
    mtu_len += pdu_len;
		if(Send_bip_Flag_COV)
		{
			uip_ipaddr_t addr;
			if(bip_send_client_conn_COV != NULL) 
			{		
				uip_udp_remove(bip_send_client_conn_COV);
			}
			Send_bip_count_COV = MAX_RETRY_SEND_BIP;
			
			Test[18]++;
			//memcpy(&Test[14],Send_bip_address_COV,6);
			
			uip_ipaddr(addr,255,255,255,255/*Send_bip_address_COV[3],Send_bip_address_COV[2],Send_bip_address_COV[1],Send_bip_address_COV[0]*/);
			bip_send_client_conn_COV = uip_udp_new(&addr, HTONS(47808/*Send_bip_address_COV[5] * 256 + Send_bip_address_COV[4]*/)); // des port
			
			if(bip_send_client_conn_COV != NULL) 
			{ Test[19]++;
				uip_udp_bind(bip_send_client_conn_COV,HTONS(Modbus.Bip_port));  // src port	
			}
			memcpy(bip_bac_buf_COV.buf,&mtu1, mtu_len);			
			bip_bac_buf_COV.len = mtu_len;
		}
		else
			uip_send((char *)mtu, mtu_len);
...
// FOR BBMD
struct uip_udp_conn * bip_send_client_conn_BBMD;
int bip_send_pdu_client_BBMD(
    BACNET_ADDRESS * dest,      /* destination address */
    BACNET_NPDU_DATA * npdu_data,       /* network information */
    uint8_t * pdu,      /* any data to be sent - may be null */
...
{       /* number of bytes of data */  
 ...
 //   (void) npdu_data;
    ...
    mtu_len += pdu_len;
		...
		{
			uip_ipaddr_t addr;
			if(bip_send_client_conn_BBMD != NULL) 
			{		
				uip_udp_remove(bip_send_client_conn_BBMD);
			}
			Send_bip_count_BBMD = MAX_RETRY_SEND_BIP;
			uip_ipaddr(addr,Send_bip_address_BBMD[0],Send_bip_address_BBMD[1],Send_bip_address_BBMD[2],Send_bip_address_BBMD[3]);	
			bip_send_client_conn_BBMD = uip_udp_new(&addr, HTONS(Send_bip_address_BBMD[4] * 256 + Send_bip_address_BBMD[5])); // des port
			if(bip_send_client_conn_BBMD != NULL) 
			{ 
				uip_udp_bind(bip_send_client_conn_BBMD,HTONS(Modbus.Bip_port/*UDP_BACNET_LPORT*/));  // src port	
			}
			memcpy(bip_bac_buf_BBMD.buf,&mtu1, mtu_len);			
			bip_bac_buf_BBMD.len = mtu_len;
...
uint16_t get_own_network_number(void)
{
	return Modbus.network_number;
}

extern Byte	Station_NUM;
U8_T flag_network_router;
    ...
    BACNET_ADDRESS * my_address)
{
    ...
    if (my_address) {
        ...
        my_address->net = Modbus.network_number;    /* local only, no routing */
				...
				{
					if(flag_network_router == 1) // the src is from router, added by chelsea
						my_address->len = 0; 
					else
					...
					#endif
					//memcpy(&my_address->adr[0], &Modbus.ip_addr, 4);
//					my_address->adr[4] = 0xba;
//					my_address->adr[5] = 0xc0;
					//memcpy(&my_address->adr[4], &BIP_Socket, 2);
					memcpy(&my_address->adr[0], &Modbus.mac_addr, 6);
				}
    }
