...
void scd4x_calibration(uint16_t value);
void check_task(void);
extern uint8_t flag_suspend_mstp;
extern uint16_t count_suspend_mstp;
extern uint16_t READ_POINT_TIMER;
extern uint16_t READ_POINT_TIMER_FROM_EEP;
extern int8_t read_sub_offset;

#if  ARM_TSTAT_WIFI    
extern u16 voltage_occ;
U8_T far clear_high_spd[HI_COMMON_CHANNEL];
extern int16 HumSensor_dew_pt;
extern int16 HumSensor_dew_pt_F;
extern uint16 HumSensor_Pws;
extern uint16 HumSensor_Mix_Ratio;
extern uint16 HumSensor_Enthalpy;
#endif

#if ARM_TSTAT_WIFI || ARM_MINI
extern uint8 flag_connect_AP;
extern uint8_t flag_set_wifi;
void save_suspend_mstp(void);
...
extern U8_T show_identify;
extern U8_T count_show_id;
...
// dont use it now, T3000 will change baut 
U8_T far scan_port;
U8_T far scan_baut;
...
#if ARM_TSTAT_WIFI
U32_T far Input_RPM[HI_COMMON_CHANNEL + 4];
U32_T far high_spd_counter[HI_COMMON_CHANNEL + 4];
U32_T far high_spd_counter_tempbuf[HI_COMMON_CHANNEL + 4];
U8_T far high_spd_en[HI_COMMON_CHANNEL + 4];
#else
...
U8_T flag_change_realy;
U8_T last_relay_status;
U8_T count_change_relay;
...
void check_TXEN(void)
{
	...
	{
#if (ARM_MINI || ARM_CM5)
		if((Modbus.mini_type == MINI_TINY_ARM) || (Modbus.mini_type == MINI_TINY_11I)
		|| (Modbus.mini_type == MINI_NANO))
		{
			if(UART0_TXEN_TINY == SEND)
				count1++;
			else
				count1 = 0;
		}
...
U16_T wait_subnet_response(U16_T nDoubleTick,U8_T port)
{
	...
	for(i = 0; i < nDoubleTick; i++)
	{
		...
		vTaskDelay(2);
#if ARM_MINI
		/* Refresh TCP TTL while blocked waiting on RS485 (multi-ID poll). */
		{
			extern void tcpip_rs485_keepalive(void);
			tcpip_rs485_keepalive();
		}
#endif
#endif	
...
void modubs_main_uart_int_hander()
{
	...
	{		
...
#endif
		{
			...
			if(max_buf[0] > MAX_BUF_LEN) 
				max_buf[0] = MAX_BUF_LEN;  
...
//				uart0_serial_receive_timeout_count = 0;
		...
		{		
			...
			{
				uint16_t count = 0;
				while((USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET) && (count++ < 10000)) ;
...
//				uart1_serial_receive_timeout_count = 0;
...
#endif
		{
...
#if (ARM_MINI || ARM_CM5 || ARM_TSTAT_WIFI )
			{
				...
				{
					uint16_t count = 0;
					while(USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET) count++;
					USART_ClearFlag(USART2, USART_FLAG_TC);
	...
	else if(Modbus.main_port == 2)
	{
		if(USART_GetITStatus(USART3, USART_IT_RXNE) == SET)   //????D??
		{
			...
			if(max_buf[2] > MAX_BUF_LEN) 
				max_buf[2] = MAX_BUF_LEN; 
...
uint8_t CRC8( uint8_t * buf)
{
    uint8_t crc = 0xff; /* accumulates the crc value */
//    uint8_t frame_crc;  /* appended to the end of the frame */

    crc = CRC_Calc_Header(buf[2], crc);
    crc = CRC_Calc_Header(buf[3], crc);
    crc = CRC_Calc_Header(buf[4], crc);
    crc = CRC_Calc_Header(buf[5], crc);
    crc = CRC_Calc_Header(buf[6], crc);
		crc = CRC_Calc_Header(buf[7], crc);
...
void mstp_ptp_int_handler(uint8_t port)
{
		...
		if(port == 2)
		{
...
#if 1
			// check whether mstp error
				if(uart2_rece_count < 250)
				{
					uart2_data_buffer[uart2_rece_count++] = data_byte;//(USART1->DR);      //??????????Y
					
					if(uart2_data_buffer[uart2_rece_count - 1] == 0x55)
					{
						uart2_data_buffer[0] = 0x55;
						uart2_rece_count = 1;
					}
					if(uart2_rece_count == 8)
					{
						if(uart2_data_buffer[0] == 0x55 && uart2_data_buffer[1] == 0xff)
						{
							uint8_t ret;
							ret = CRC8(uart2_data_buffer);
							if(ret == 0)
							{
								flag_mstp_err[2] = 0;
							}
			...
			if( USART_GetITStatus(USART3, USART_IT_TXE) == SET  )		
			{
				...
				{
					uint16_t count = 0;
					while(USART_GetFlagStatus(USART3, USART_FLAG_TC) == RESET) count++;
					USART_ClearFlag(USART3, USART_FLAG_TC);
					USART_ITConfig(USART3, USART_IT_TXE, DISABLE);
					uart2_send_count = 0;
					uart2_sendbyte_num = 0;
		...
		else if(port == 0)
		{
...
#if 1
				// check whether mstp error
				if(uart0_rece_count < 250)
				{
					uart0_data_buffer[uart0_rece_count++] = data_byte;//(USART1->DR);      //??????????Y
					
					if(uart0_data_buffer[uart0_rece_count - 1] == 0x55)
					{
						uart0_data_buffer[0] = 0x55;
						uart0_rece_count = 1;
					}
					if(uart0_rece_count == 8)
					{
						if(uart0_data_buffer[0] == 0x55 && uart0_data_buffer[1] == 0xff)
						{
							uint8_t ret;
							ret = CRC8(uart0_data_buffer);
							if(ret == 0)
							{	
								flag_mstp_err[0] = 0;
							}							
			...
			if( USART_GetITStatus(USART1, USART_IT_TXE) == SET  )		
			{
				...
				{
					uint16_t count = 0;
					while(USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET) count++;
					Test[42] = count;
					USART_ClearFlag(USART1, USART_FLAG_TC);
					USART_ITConfig(USART1, USART_IT_TXE, DISABLE);
					uart0_send_count = 0;
...
void modubs_sub1_uart_int_hander(void)
{
	...
	{	
		...
		// check whether receive mstp packets
			if(uart0_data_buffer[0] == 0x55 && uart0_data_buffer[1] == 0xff && uart0_data_buffer[2] == 0x01 && uart0_data_buffer[5] == 0x00 && uart0_data_buffer[6] == 0x00)
			{//bacnet protocal detected
				if(upate_mstp_flag != 0) // updating sub mstp devices
				{
					upate_mstp_flag = 1;
...
/* add transaction id */ 	
	...
	{			
		...
		//			}	
					...
					else if(StartAdd + loop == MODBUS_NETWORK_MASTER)
					{
						sendbuf[HeadLen + 3 + loop * 2] = 0;
						sendbuf[HeadLen + 3 + loop * 2 + 1] = Modbus.network_master;
					}
					else if(StartAdd + loop == MODBUS_MSTP_MASTER)
					{
						sendbuf[HeadLen + 3 + loop * 2] = 0;
						sendbuf[HeadLen + 3 + loop * 2 + 1] = Modbus.mstp_master;
					}
					else if(StartAdd + loop == MODBUS_SYNC_UDP_SCAN_LPORT)
					{
						sendbuf[HeadLen + 3 + loop * 2] = udp_scan_lport >> 8;
						sendbuf[HeadLen + 3 + loop * 2 + 1] = udp_scan_lport;
					}
...
//					}
					...
					else if(StartAdd + loop == MODBUS_UPDATE_MSTP_DEVICE)
					{
						sendbuf[HeadLen + 3 + loop * 2] = 0;
						sendbuf[HeadLen + 3 + loop * 2 + 1] = upate_mstp_flag;
					}
...
#if ARM_TSTAT_WIFI					
					else if(StartAdd + loop == MODBUS_ICON_CONFIG)
					{
						sendbuf[HeadLen + 3 + loop * 2] = 0;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = (U8_T)Modbus.icon_config;
					}
		...
		//			}
					...
					{				
						sendbuf[HeadLen + 3 + loop * 2] = count_write_Flash >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] =  count_write_Flash;
					}
		...
		//			}
					...
					{
						sendbuf[HeadLen + 3 + loop * 2] = test_adc_flag >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = test_adc_flag; 	
					}
					else if(StartAdd + loop == MODBUS_READ_POINT_TIMER)
					{
						sendbuf[HeadLen + 3 + loop * 2] = READ_POINT_TIMER >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = READ_POINT_TIMER; 	
					}
					else if(StartAdd + loop == MODBUS_READ_SUB_OFFSET)
					{
						sendbuf[HeadLen + 3 + loop * 2] = 0;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = read_sub_offset; 	
					}
					...
					else if(StartAdd + loop == MODBUS_TEMPERATURE)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] =  (inputs[HI_COMMON_CHANNEL2].value / 100) >> 8; 
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2].value / 100); 	
						}
						else
						{
					...
					else if(StartAdd + loop == MODBUS_TVOC)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL2 + 1].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2 + 1].value / 1000);	
						}
						else
						{
					...
					else if(StartAdd + loop == MODBUS_HUMIDY)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL2 + 2].value / 100) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2 + 2].value / 100);	
						}
						else
						{
					...
					else if(StartAdd + loop == MODBUS_CO2) // HI_COMMON_CHANNEL + 4
					{
						if(Modbus.mini_type == MINI_T10P)
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL + 4].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL + 4].value / 1000);	
						}
						else if(Modbus.mini_type == MINI_T3OEM_12I)
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL2 + 4].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2 + 4].value / 1000);	
						}
						else
						{
							sendbuf[HeadLen + 3 + loop * 2] =  (inputs[COMMON_CHANNEL + 4].value / 1000) >> 8; 
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[COMMON_CHANNEL + 4].value / 1000); 	
						} 	
					}					
					else if(StartAdd + loop == MODBUS_LIGHT)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL2 + 5].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2 + 5].value / 1000);	
						}
						else
						{
					...
					else if(StartAdd + loop == MODBUS_VOICE)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[HI_COMMON_CHANNEL2 + 6].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[HI_COMMON_CHANNEL2 + 6].value / 1000);
						}
						else
						{
					...
					else if(StartAdd + loop == MODBUS_PULSE1)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[12].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[12].value / 1000);
						}
						else
						{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
					...
					else if(StartAdd + loop == MODBUS_PULSE2)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[12 + 1].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[12 + 1].value / 1000);
						}
						else
						{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
					...
					else if(StartAdd + loop == MODBUS_PULSE3)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[12 + 2].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[12 + 2].value / 1000);
						}
						else
						{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
					...
					else if(StartAdd + loop == MODBUS_PULSE4)
					{
						...
						{
							sendbuf[HeadLen + 3 + loop * 2] = (inputs[12 + 3].value / 1000) >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = (inputs[12 + 3].value / 1000);
						}
						else
						{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
					...
					else if(StartAdd + loop == MODBUS_PTERM1)
					{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = controllers[0].proportional;
					}
					else if(StartAdd + loop == MODBUS_ITERM1)
					{
							sendbuf[HeadLen + 3 + loop * 2] = 0;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = controllers[0].reset;
					}
					else if(StartAdd + loop == MODBUS_DEW_PT)
					{
							sendbuf[HeadLen + 3 + loop * 2] = HumSensor_dew_pt >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = HumSensor_dew_pt;
					}
					else if(StartAdd + loop == MODBUS_DEW_PT_F)
					{
							sendbuf[HeadLen + 3 + loop * 2] = HumSensor_dew_pt_F >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = HumSensor_dew_pt_F;
					}
					else if(StartAdd + loop == MODBUS_PWS)
					{
							sendbuf[HeadLen + 3 + loop * 2] = HumSensor_Pws >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = HumSensor_Pws;
					}
					else if(StartAdd + loop == MODBUS_MIX_RADIO)
					{
							sendbuf[HeadLen + 3 + loop * 2] = HumSensor_Mix_Ratio >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = HumSensor_Mix_Ratio;
					}
					else if(StartAdd + loop == MODBUS_ENTHALPY)
					{
							sendbuf[HeadLen + 3 + loop * 2] = HumSensor_Enthalpy >> 8;	
							sendbuf[HeadLen + 3 + loop * 2 + 1] = HumSensor_Enthalpy;
					}
					else if(StartAdd + loop == MODBUS_VCC_ADC_1)
					{
						sendbuf[HeadLen + 3 + loop * 2] = new_vcc_adc[0] >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = new_vcc_adc[0]; 	
					}
					else if(StartAdd + loop == MODBUS_VCC_ADC_2)
					{
						sendbuf[HeadLen + 3 + loop * 2] = new_vcc_adc[1] >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = new_vcc_adc[1]; 	
					}
					else if(StartAdd + loop == MODBUS_VCC_ADC_3)
					{
						sendbuf[HeadLen + 3 + loop * 2] = new_vcc_adc[2] >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] = new_vcc_adc[2]; 	
					}				
#endif	
// only for PLC, adjuct AO2,AO3
#if ADJUST_NG2_AO	
					else if(StartAdd + loop >= MODBUS_OUTPUT_1V_2 && StartAdd + loop <= MODBUS_OUTPUT_10V_2)
					{
						sendbuf[HeadLen + 3 + loop * 2] = new_start_adc[0][StartAdd + loop - MODBUS_OUTPUT_1V_2 + 1] >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] =  new_start_adc[0][StartAdd + loop - MODBUS_OUTPUT_1V_2 + 1];
					}
					else if(StartAdd + loop >= MODBUS_OUTPUT_1V_3 && StartAdd + loop <= MODBUS_OUTPUT_10V_3)
					{
						sendbuf[HeadLen + 3 + loop * 2] = new_start_adc[1][StartAdd + loop - MODBUS_OUTPUT_1V_3 + 1] >> 8;	
						sendbuf[HeadLen + 3 + loop * 2 + 1] =  new_start_adc[1][StartAdd + loop - MODBUS_OUTPUT_1V_3 + 1];
					}
#endif					
...
//					else if(StartAdd + loop == MODBUS_UART0_WORDLEN)
//					{
//						sendbuf[HeadLen + 3 + loop * 2] = 0;	
//						sendbuf[HeadLen + 3 + loop * 2 + 1] = Modbus.uart_WordLen[0]; 	
//					}
//					else if(StartAdd + loop == MODBUS_UART1_WORDLEN)
//					{
//						sendbuf[HeadLen + 3 + loop * 2] = 0;	
//						sendbuf[HeadLen + 3 + loop * 2 + 1] = Modbus.uart_WordLen[1]; 	
//					}
//					else if(StartAdd + loop == MODBUS_UART2_WORDLEN)
//					{
//						sendbuf[HeadLen + 3 + loop * 2] = 0;	
//						sendbuf[HeadLen + 3 + loop * 2 + 1] = Modbus.uart_WordLen[2]; 	
//					}			
...
//#endif
		...
		else if(StartAdd == MODBUS_ADDRESS)
		{
			...
			dlmstp_init(NULL);
#endif
			//E2prom_Read_Byte(EEP_ADDRESS,&Modbus.address);  // for test E2PROM
		}
		...
		else if(StartAdd == MODBUS_UPDATE_STATUS)
		{
			...
			{
				// save whether suspend MSTP before jump to ISP
				save_suspend_mstp();
...
//					Modbus.serialNum[3] = 0;
			...
			else if(update_flash == 0x55)
			{				
				flag_suspend_mstp = 1;
				count_suspend_mstp = pData[HeadLen + 4] * 60;
				// save whether suspend MSTP before jump to ISP
				save_suspend_mstp();
			}
	...
	if((Modbus.mini_type == MINI_BIG_ARM) || (Modbus.mini_type == MINI_SMALL_ARM))
	{
		if(Modbus.com_config[1] == MODBUS_MASTER)
			...
			UART1_SW = 0;
				...
				com_config_back[i] = Modbus.com_config[i];
				flag_suspend_mstp = 0;
		...
		{
			if(pData[HeadLen + 5] == 0 || pData[HeadLen + 5] >= 60)
			{
...
//		}	
		...
		{
			test_adc_flag = pData[HeadLen + 5] + (pData[HeadLen + 4]<<8);
		}
		else if(StartAdd == MODBUS_READ_POINT_TIMER)
		{
			READ_POINT_TIMER = pData[HeadLen + 5] + 256 * pData[HeadLen + 4];
			READ_POINT_TIMER_FROM_EEP = READ_POINT_TIMER;
			E2prom_Write_Byte(EEP_READ_POINT_TIMER, READ_POINT_TIMER / 100);
		}
		else if(StartAdd == MODBUS_READ_SUB_OFFSET)
		{
			read_sub_offset = pData[HeadLen + 5];
			E2prom_Write_Byte(EEP_READ_SUB_OFFSET,read_sub_offset);			
		}
...
#if ARM_TSTAT_WIFI
		else if(StartAdd == MODBUS_CO2_CALIBRATION)
		{
			scd4x_calibration(pData[HeadLen + 5] + 256 * pData[HeadLen + 4]);
		}
#endif
...
//		}
		...
		else if(StartAdd == MODBUS_SYNC_UDP_SCAN_LPORT)
		{
			if(pData[HeadLen + 5] == 1)
				udp_scan_lport = 1235;
			else
			...
			E2prom_Write_Byte(EEP_SYNC_UDP_SCAN_SEND_LPORT,pData[HeadLen + 5]);
		}
...
//		}
		...
		else if(StartAdd == MODBUS_UPDATE_MSTP_DEVICE)
		{
			upate_mstp_flag = pData[HeadLen + 5];
			if(upate_mstp_flag == F_START_SHUTDOWN)
			{
//				if(Modbus.com_config[2] == MODBUS_MASTER)
//					Modbus.com_config[2] = BACNET_MASTER;
...
//					Modbus.com_config[0] = BACNET_MASTER;
				if(pData[HeadLen + 4] != 0)
					shutdown_Time = pData[HeadLen + 4];
				else
					shutdown_Time = 5; // default shutdown time
			
				shutdown_start_time = run_time;
				shutdown_send_time = run_time;
				
//				flag_suspend_mstp = 1;
//				count_suspend_mstp = shutdown_Time * 60;
//				// save whether suspend MSTP before jump to ISP
//				save_suspend_mstp();
			}
		}
...
//			AX11000_SoftReboot();
...
#if ADJUST_NG2_AO			
		else if(StartAdd >= MODBUS_OUTPUT_1V_2 && StartAdd <= MODBUS_OUTPUT_10V_2)
		{
			new_start_adc[0][StartAdd - MODBUS_OUTPUT_1V_2 + 1] = pData[HeadLen + 5]+ (pData[HeadLen + 4]<<8);		
			E2prom_Write_Byte(EEP_OUT_1V_2 + (StartAdd - MODBUS_OUTPUT_1V_2) * 2, pData[HeadLen + 4]);
			E2prom_Write_Byte(EEP_OUT_1V_2 + (StartAdd - MODBUS_OUTPUT_1V_2) * 2 + 1, pData[HeadLen + 5]);
			// caclulate slop
			cal_slop();
		}
		else if(StartAdd >= MODBUS_OUTPUT_1V_3 && StartAdd <= MODBUS_OUTPUT_10V_3)
		{
			new_start_adc[1][StartAdd - MODBUS_OUTPUT_1V_3 + 1] = pData[HeadLen + 5]+ (pData[HeadLen + 4]<<8);		
			E2prom_Write_Byte(EEP_OUT_1V_3 + (StartAdd - MODBUS_OUTPUT_1V_3) * 2, pData[HeadLen + 4]);
			E2prom_Write_Byte(EEP_OUT_1V_3 + (StartAdd - MODBUS_OUTPUT_1V_3) * 2 + 1, pData[HeadLen + 5]);
			// caclulate slop
			cal_slop();
		}
#endif
		...
		else if(StartAdd == MODBUS_PTERM1)
		{
			controllers[0].proportional = pData[HeadLen + 5];
#if (ARM_MINI || ARM_CM5 || ARM_TSTAT_WIFI )
			write_page_en[CON] = 1;	
#endif
			ChangeFlash = 1;
		}
		else if(StartAdd == MODBUS_ITERM1)
		{
			controllers[0].reset = pData[HeadLen + 5];
#if (ARM_MINI || ARM_CM5 || ARM_TSTAT_WIFI )
			write_page_en[CON] = 1;	
#endif
			ChangeFlash = 1;
...
#endif
//		else if(StartAdd == MODBUS_UART0_WORDLEN)
//		{
//			Modbus.uart_WordLen[0] = pData[HeadLen + 5];
//			E2prom_Write_Byte(EEP_UART0_WORDLEN,pData[HeadLen + 5]);
//		}
//		else if(StartAdd == MODBUS_UART2_WORDLEN)
//		{
//			Modbus.uart_WordLen[2] = pData[HeadLen + 5];
//			E2prom_Write_Byte(EEP_UART2_WORDLEN,pData[HeadLen + 5]);
//		}
#if (ASIX_MINI || ASIX_CM5)
#if USB_HOST
...
/********************************************************\*/		
		...
		{			
			...
			if(StartAdd - MODBUS_TEST == 47)
				AT24CXX_WriteOneByte(EEP_RAM_ERR, Test[47]);
			if(StartAdd - MODBUS_TEST == 48)
				AT24CXX_WriteOneByte(EEP_TEST1, Test[48]);
			if(StartAdd - MODBUS_TEST == 49)
				...
				if(outputs[i].digital_analog == 0)  // digital
				{
					...
					if(i < get_max_internal_output())
					{
						...
						{ // inverse
							...
							last_relay_status = outputs[i].control;// if block-wirte, we should do special dealwith
						...
						// if block-wirte, we should do special dealwith
							if(last_relay_status != outputs[i].control)
							{
								flag_change_realy = 1;
								count_change_relay = 0;
							}
					}
					else
					{
...
#if ADJUST_NG2_AO	
		else if(StartAdd == MODBUS_OUTPUT_1V_2)
		{
			if(pData[HeadLen + 6] == 20)
			{
				char i;
				for(i = 1;i <= 10;i++)
				{
					new_start_adc[0][i] = (U16_T)(pData[HeadLen + i * 2 + 5] << 8) + pData[HeadLen + i * 2 + 6];
					E2prom_Write_Byte(EEP_OUT_1V_2 + i * 2 - 2, pData[HeadLen + i * 2 + 5]);
					E2prom_Write_Byte(EEP_OUT_1V_2 + i * 2 - 1, pData[HeadLen + i * 2 + 6]);
			
				}
				cal_slop();				
			}
		}
		else if(StartAdd == MODBUS_OUTPUT_1V_3)
		{
			if(pData[HeadLen + 6] == 20)
			{
				char i;
				for(i = 1;i <= 10;i++)
				{
					new_start_adc[1][i] = (U16_T)(pData[HeadLen + i * 2 + 5] << 8) + pData[HeadLen + i * 2 + 6];
					E2prom_Write_Byte(EEP_OUT_1V_3 + i * 2 - 2, pData[HeadLen + i * 2 + 5]);
					E2prom_Write_Byte(EEP_OUT_1V_3 + i * 2 - 1, pData[HeadLen + i * 2 + 6]);
...
//			update_timers();
		...
		{ 
			if((ptr->reg.en_sntp == 0) && 
				(Modbus.mini_type == MINI_TSTAT10 || Modbus.mini_type == MINI_T10P || Modbus.mini_type == MINI_T3OEM_12I))
			{
...
//		if(Modbus.uart_WordLen[0] != ptr->reg.uart_WordLen[0])
//		{
//			Modbus.uart_WordLen[0] = ptr->reg.uart_WordLen[0];
//			E2prom_Write_Byte(EEP_UART0_WORDLEN, Modbus.uart_WordLen[0]);
//			flag_initial_uart = 1;//UART_Init(0);
//		}
//		if(Modbus.uart_WordLen[2] != ptr->reg.uart_WordLen[2])
//		{
//			Modbus.uart_WordLen[2] = ptr->reg.uart_WordLen[2];
//			E2prom_Write_Byte(EEP_UART2_WORDLEN, Modbus.uart_WordLen[2]);
//			flag_initial_uart = 3;
//		}
...
// reboot mstp communication		
		...
		if(Modbus.network_number != (ptr->reg.network_number + 256L * ptr->reg.network_number_hi))
		{
			...
			E2prom_Write_Byte(EEP_NETWORK_NUMBER_LO,ptr->reg.network_number);
			E2prom_Write_Byte(EEP_NETWORK_NUMBER_HI,ptr->reg.network_number_hi);
		}
		if(mstp_network != Setting_Info.reg.mstp_network_number)
		{
			mstp_network = Setting_Info.reg.mstp_network_number;
			E2prom_Write_Byte(EEP_MSTP_NETWORK_LO,(U8_T)mstp_network);
		...
		if(Modbus.refresh_flash_timer != ptr->reg.write_flash)
		{ 
			if(ptr->reg.write_flash == 0 || ptr->reg.write_flash >= 60)
			{
				Modbus.refresh_flash_timer = ptr->reg.write_flash;
				E2prom_Write_Byte(EEP_REFRESH_FLASH, Modbus.refresh_flash_timer );
				if(Modbus.refresh_flash_timer == 0)
					ChangeFlash = 0;
				else
		...
		// if change baudrate via com port, do not initial uart port at once, response T3000 first...
...
//		}
		...
		{			
			...
			E2prom_Write_Byte(EEP_JASON, ptr->reg.webview_json_flash);
		}
...
void QuickSoftReset(void)
{
	...
	/* Never flush corrupted OUT/IN/VAR — restore from flash first */
