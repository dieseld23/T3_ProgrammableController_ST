...
//	GPIO_InitTypeDef GPIO_InitStructure;
	...
	RCC->APB2ENR |= (1 << 4) | (1 << 5);    //Ê¹ÄÜPORTC,PORTDÊ±ÖÓ	   	 
	/* Configure PC.08, PC.09, PC.10, PC.11, PC.12 pin: D0, D1, D2, D3, CLK pin */  	 
	GPIOC->CRH &= 0XFFF00000; 
	GPIOC->CRH |= 0X000BBBBB;				//PC8,PC9,PC10,PC11,PC12¸´ÓÃÍÆÍìÊä³ö   	 
    GPIOC->ODR |= (1 << 8) | (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12);//³õÊ¼Êä³ö¸ß
	/* Configure PD2,    pin: cmd */										  
	GPIOD->CRL &= 0XFFFFF0FF;
	GPIOD->CRL |= 0X00000B00;				//PD.2¸´ÓÃÍÆÍìÊä³ö
	GPIOD->ODR |= (1 << 2);
 	...
 	else if((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
	{
 		...
 		else   
		{
 			...
 			if(SD_OK == errorstatus)
			{
				...
				SDIO->CLKCR |= (u16)wmode << 11;	//1Î»/4Î»×ÜÏß¿í¶È 
...
SD_Error SD_Init(void)
{
   	...
   	if(errorstatus == SD_OK)
	{
		...
		errorstatus = SD_EnableWideBusOperation(1);			//4Î»¿í¶È,Èç¹ûÊÇMMC¿¨,Ôò²»ÄÜÓÃ4Î»Ä£Ê½ 
	}
	
  	if((errorstatus == SD_OK) || (SDIO_MULTIMEDIA_CARD == CardType))
	{  		    
		SDIO_Clock_Set(SDIO_TRANSFER_CLK_DIV + 5);				//ÉèÖÃÊ±ÖÓÆµÂÊ,SDIOÊ±ÖÓ¼ÆËã¹«Ê½:SDIO_CKÊ±ÖÓ=SDIOCLK/[clkdiv+2];ÆäÖÐ,SDIOCLKÒ»°ãÎª72Mhz 
