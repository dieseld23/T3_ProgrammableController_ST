...
def icon_names(src):
    """The literal RGB565 icons.  The 4 bit icons' palettes are uint16
    arrays too, but they hold colours rather than pixels, so
    recolour_icons.py would blend them into mush -- they are excluded here
    and regenerated by icongen.py, which reads the background itself."""
    return [n for n in re.findall(r'uint16 const (\w+)\[\]\s*=', src)
            if not n.startswith('pal_')]
