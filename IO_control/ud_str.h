...
#define MAXNETWORKPOINTS       128//32
#define MAXLOCALPOINTS				 20//128
...
#define MAX_VAR_UNIT 5
#define VAR_UNIT_SIZE 20
...
#define MB_COIL_REG 23
#define MB_DIS_REG 	24
#define MB_IN_REG 	25
#define MB_REG 			26
...
#define MB_HOLDING_FLOAT_ABCD  32
#define MB_HOLDING_FLOAT_CDAB  33
#define MB_HOLDING_FLOAT_BADC  34
#define MB_HOLDING_FLOAT_DCBA  35

#define MB_INPUT_FLOAT_ABCD  36
#define MB_INPUT_FLOAT_CDAB  37
#define MB_INPUT_FLOAT_BADC  38
#define MB_INPUT_FLOAT_DCBA  39


typedef enum
	{
		OUT=0, IN, VAR, CON, WRT, AR, PRG,PVAR = 7,/* TBL,*/  TZ = 8,
...
//     READTOTALIZER_T3000       = TZ+1,         /* read totalizers     */
		 ...
		 READARRAY_T3000           = ARRAY+1, //12     /* read arrays         */
		 READARRAYVALUE_T3000      = 30,    /* read array elements */
...
//     WRITETOTALIZER_T3000      = 100+TZ+1,         /* write totalizers     */
		 ...
		 WRITEARRAY_T3000          = 100+ARRAY+1,  //112    /* write arrays         */
		 WRITEARRAYVALUE_T3000     = 130,    /* write array elements */
...
typedef enum { not_used_output, V0_10, P0_100_Open, P0_20psi, P0_100,
						P0_100_Close, I_0_20ma , P0_100_PWM,P0_100_2_10V}Analog_output_range_equate;
...
typedef struct
{
	...
	// if point_type is MB_REG, last 5 bits are for regisger number
}	Point_Net;


typedef struct
{
	uint32_t identification; //0x55 0xff 0x55 0xff
	uint8_t instance_high_byte;
	uint8_t instance_mid_byte;
	uint8_t instance_low_byte;
	uint8_t point_type;
	uint8_t obj_number_high;
	uint8_t obj_number_mid;
	uint8_t obj_number_low;
	uint8_t reserve[3];
}Point_Bacnet;
...
typedef struct
{
	...
	int8_t auto_manual;  /* (1 bit; =o, 1=manual)*/
	...
	int8_t out_of_service;//read_remote;//digital_control;  /* (1 bit)*/
...
typedef struct
{
	...
	U8_T element_count;//update;                /* refresh time */
...
{
	S8_T	table_name[8];
	U8_T 	precision; // 0xef -> 0.01, other 0.1
	Tbl_point		dat[16];
...
//	U8_T change	;//			       : 2;
	U8_T time_to_live;
	U32_T instance;
}	POINTS_HEADER; /* 1+5+4+2+2=14 bytes */
...
#define NP_TIME_TO_LIVE 20   	// 10s * point_number    network modbus points & network bacnet points
...
typedef union
{
 ...
 struct
 {
	uint8_t label_status;   // 0: unused    1: occupied  2: deleted
  uint32_t nSerialNum;
  uint8_t nScreen_index;  // which group
  ...
  uint8_t nIcon_size;
  uint8_t nIcon_place;
  S8_T icon_name_1[STR_ICON_1_NAME_LENGTH];
...
typedef struct {
	...
	Uint Length;
	uint8_t HeaderCRC;
	S8_T Buffer[MAXFRAMEBUFFER+2+1];
	Uint DataCRC;
	uint8_t being_used ;
	uint8_t locked ;
	uint8_t port ;
