...
{		if(point == 3) Test[0]++;
	...
	else if(input_type[point] == INPUT_PT1K)
	{ // for special PRESSURE sensor (PLC)
		return ( 3000L  * sample ) >> 10;
	}
...
{	
	...
	{
		return T3OEM_12I_MAX_AOS + T3OEM_12I_MAX_DOS;
	}
	return 0;
}
...
void Store_Pulse_Counter(uint8 flag)
{
	uint16 save_time;
#if ARM_TSTAT_WIFI
	static u32 old_pulse[HI_COMMON_CHANNEL + 4];
#else
...
{
#if ARM_TSTAT_WIFI
	static u32 old_count[HI_COMMON_CHANNEL + 4];
	static u32 runtime[HI_COMMON_CHANNEL + 4];
	static u16 count[HI_COMMON_CHANNEL + 4]; 
#else
...
void Check_spd_count_led(void)
{
	char i;
#if ARM_TSTAT_WIFI
	static u32 old_count2[HI_COMMON_CHANNEL + 4];
#else
